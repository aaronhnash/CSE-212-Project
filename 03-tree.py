...
def solve_maze(maze, x=0, y=0, curr_path=None):
    ...
    if is_valid_move(maze,curr_path,x+1,y): # can I move positively to the right?
        # If I can move positively to the right, why don't I try calling myself again?
        solve_maze(maze, x+1, y, curr_path)
        #If I get past this point, then it means that this step led to a dead end. I'll remove myself and try with the next step.
        curr_path.pop()
...
# Example 2: Searching through a BST
"""
CSE 212 Lesson 9A
...
class BST:
    ...
    def contains(self, value):
        """A function that initiates a search through the BST to see if a certain value is contained within. """
        current_node = self.root
        found = self._search_bst(value, current_node)
        return found
    
    def _search_bst(self, value, curr):
        """The internal function that calls itself recursively. """
        # Remember: you want this function to call itself until it reaches "the end". 
        # There are a few ways to do this, so don't be afraid to try!

        # Hints: 
        # Am I at the end of the tree, ie. curr == None ?
        # Is curr.data the same as value ?
        # Is it less than value ? If so, I should look to the left.
        # if it's more than value, I should look to the right.
        # If the value is found, then this function should return True and carry it all the way up.
        pass
